"""Execute every example on the docsite's narrative pages.
...
Markdown source, so this needs the built cdylib and nothing else.
"""
...
def main():
    ...
    pages = sorted((ROOT / "docsite").glob("*.md")) + sorted(
        (ROOT / "docsite" / "api").glob("*.md")
    )
    ...
    if failed:
        print(f"\ndocsite examples: {failed} of {examples} failed")
        return 1
    print(
        f"docsite examples: {examples} pass, over {fenced_pages} pages "
        f"of {len(pages)}"
    )
    return 0
