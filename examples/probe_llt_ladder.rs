...
//! LLT polynomials — `Rise` and its weights `t^{−a_i}` are functions of the area
//! sequence alone, so the `z`-extraction factors out of the labelling sum (see
//! `dyck.rs`'s module docs and `docs/spec-llt.md` §7.7). The labelled walk
//! survives as `dyck::ladder_at_content`, which is both the correctness oracle
//! (`the_rise_ladder_via_llt_agrees_with_the_labelled_walk`) and the cheaper
//! route for a single coarse content.
//!
...
//! compare to `docs/spec-llt.md` §6's mains table.
