...
//! and refusing.
//!
//! Two limits bound that contract and are stated rather than left to be
//! discovered. The release profile sets no `overflow-checks`, so a
//! fixed-width path that neither escalates nor checks has no backstop beneath
//! it in a release build. And the `(q,t)` families' walls are unmeasured: a
//! family that does not state its reach has not measured it.
//!
//! `docs/policies/failure.md` is the rulebook this compresses — which
//! mechanism each situation demands, and where current practice diverges from
//! it.
//!
//! ## The model
//!
