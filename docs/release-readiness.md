...
- [x] Same exercise for Python: ⚠️ **98 entry points**, not the 87 this file
      ...
      **All 98 are supported and none is harness-only** — a result, not a
...
bringing each of the 98 docstrings to [style.md](style.md)'s checklist as P11
...
- [x] `symfn.pyi` type stubs. The API is coarse-grained and takes
      ...
      all 98 entry points, named type aliases where a Rust alias already drew
...
- It raises the stakes on **Phase 2**. The bulk/indexed entry points the shim
  ...
  They need to be a deliberate, documented, stable subset of the 98 entry
  points, not whatever happened to be exported. Phase 2 made that list — all
  98, with `symfn.pyi` and `scripts/check_python_stubs.py` holding it — so what
