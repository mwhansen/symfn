...
Membership is decided, not accumulated: **98 entry points** exist, and each is
...
The sort ran at 98 and **the harness-only set came out empty**, which is a
...
1. **Sort the 91 (P10, P6, P11).** *Membership done; the docstring sweep is
   not.* All **98** are supported and none is harness-only (P10 above records
